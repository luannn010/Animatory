...
async def test_reachable_returns_without_waking(monkeypatch):
    ...
    monkeypatch.setattr("animatory.genimage.zimage.brain.BrainClient.wake",
...
async def test_unreachable_and_wake_fails_raises(monkeypatch):
    ...
    monkeypatch.setattr("animatory.genimage.zimage.brain.BrainClient.wake", lambda self, model=None: False)
...
async def test_wake_brings_chat_up(monkeypatch):
    ...
    monkeypatch.setattr("animatory.genimage.zimage.brain.BrainClient.wake", lambda self, model=None: True)
